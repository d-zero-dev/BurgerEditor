...
**注意: ネストセレクタのデフォルト値の優先度**

ネストされたセレクタ（例: `.parent { [data-bge-container] { ... } }`）内で定義されたバリアント（`--key` 付き）は通常通り検出される。ただし、**デフォルト値の判定** においてはネストセレクタの定義は直接セレクタより低い優先度を持つ。詳細は「デフォルト値の判定」セクションを参照。
...
### ネストセレクタのデフォルト値: 直接セレクタが優先

ネストされたセレクタ（`.parent { [data-bge-container] { ... } }`）と直接セレクタ（`[data-bge-container] { ... }`）の両方でデフォルト値が定義されている場合、**直接セレクタの定義が優先される**。ネストセレクタの定義はレイヤー優先度に関わらず無視される。
...
@layer main {
	.c-content-main {
		[data-bge-container] {
			...
			/* ↑ ネストセレクタ: @layer main が高優先度だが、デフォルト判定では無視される */
		}
	}
}
```

結果: `default-gutter` が `isDefault: true` になる。`@layer main` が `@layer main-base` より高優先度であっても、ネストセレクタ内のデフォルト定義はグローバルなデフォルト判定に影響しない。

**理由**: ネストセレクタ（例: `.c-content-main [data-bge-container]`）はコンテキスト固有の上書きであり、特定の親要素配下でのみ適用される。ブロックオプションダイアログのセレクトボックスの初期値として使うグローバルなデフォルトには、コンテキストに依存しない直接セレクタの定義がふさわしい。

### フォールバック: ネストセレクタのみにデフォルトがある場合

直接セレクタでデフォルト値が定義されていないカテゴリでは、ネストセレクタの定義がフォールバックとして採用される。

```css
[data-bge-container] {
	--bge-options-font-color--normal: #232323;
	--bge-options-font-color--red: #c00;
	/* ↑ バリアントのみ。直接セレクタにデフォルト定義なし */
}

.c-content-main {
	[data-bge-container] {
		--bge-options-font-color: var(--bge-options-font-color--normal);
		/* ↑ ネストだが、他にデフォルト定義がないのでフォールバックとして採用 */
	}
}
```

結果: `normal` が `isDefault: true` になる。
