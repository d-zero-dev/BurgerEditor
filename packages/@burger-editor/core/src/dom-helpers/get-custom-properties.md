...
## 選択肢の除外（`null`）（`getCustomProperties` のみ）

`getCustomProperties` では、値が文字列 `null` のプロパティは結果から除外される。`getCustomProperty` および `getRepeatMinInlineSizeVariants` にはこのフィルタリングはない。
...
検出の仕組み（セレクター照合、`@layer` 優先度、`@scope` 透過など）は `getCustomProperties` と同じ。ただし `null` 値のフィルタリングは行わない。
