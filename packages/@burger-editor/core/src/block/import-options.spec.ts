...
test('auto-fit時にrepeatMinInlineSizeが設定されている場合、data-bge-containerに--variantが含まれ列数は含まれない', () => {
	const el = document.createElement('div');
	el.innerHTML = `
		<div data-bge-container-frame>
			<div data-bge-group>
	...
	expect(el.dataset.bgeContainer).toContain('--small');
	expect(el.dataset.bgeContainer).not.toContain(':3');
});

test('repeatMinInlineSizeがnullの場合、data-bge-containerに--が含まれない', () => {
	const el = document.createElement('div');
	el.innerHTML = `
		<div data-bge-container-frame>
			<div data-bge-group>
	...
	expect(el.dataset.bgeContainer).not.toContain('--');
	expect(el.dataset.bgeContainer).not.toContain(':3');
});
