...
BurgerEditorでは、ブロックベースのコンテンツ構造を採用しています。各ブロックは `data-bge-*` 属性によって構造が定義され、コンテナ、フレーム、グループ、アイテムの階層で構成されます。
...
また、カタログ定義の `id`、`classList`、`style` プロパティで指定された値は、すべてこのコンテナ要素（`data-bge-container`要素）に適用されます。
