...
- **README はセットアップ/利用手順のみ** — API リファレンス（JSDoc が正）やアーキテクチャ説明を置かない
- **ARCHITECTURE.md は索引として維持する** — 全体地図・境界と所有権・依存方向・レイヤー分離の原則・不変条件と負の知識に絞る。実装詳細の保存庫にしない（詳細の正は JSDoc）
- **ドキュメント整合チェックはこのスキルの責務** — JSDoc の網羅性・コメント原則の遵守・ARCHITECTURE.md と実装の一致を検証する（旧 `/doc` コマンドは廃止し、ここに統合した）
