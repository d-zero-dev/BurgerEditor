...
サンプル値が「無いこと」ではなく「**予約済み慣例に従っていること**」を確認する。HTML fixture・VR テストのサンプルページ・ブロック定義のサンプル値が対象になりやすい。
...
複数パッケージにまたがる変更をコミットする場合、必ず**リーフからルートへ**コミットする。依存グラフの詳細は ARCHITECTURE.md を参照。

| ティア | パッケージ                       |
| ------ | -------------------------------- |
| 0      | `utils`, `legacy`                |
| 1      | `frozen-patty`, `custom-element` |
| 2      | `core`                           |
| 3      | `inspector`, `migrator`          |
| 4      | `client`                         |
| 5      | `blocks`                         |
| 6      | `css`, `file-io`                 |
| 7      | `cli`, `local`, `runtime`        |
...
順序が不明な場合は `yarn lerna list --graph` で確認する。
...
- Conventional Commits を使用すること
  ...
  - 使用するスコープ:
    - パッケージ名（ネームスペースなし）: `blocks`, `cli`, `client`, `core`, `css`, `custom-element`, `file-io`, `frozen-patty`, `inspector`, `legacy`, `local`, `mcp-server`, `migrator`, `runtime`, `utils`
    - `repo`, `deps`, `github`
- メッセージ本文の各行は100文字以下
